...
public class ReActivateSubscriptionTests(IntegrationTestWebAppFactory factory) : IntegrationTestBase(factory)
{
    [Fact]
    public async Task ReActivateSubscription_ReturnsNotFoundWhenSubscriptionDoesNotExist()
    {
        // Arrange
        ...
        // Act
        var response = await Client.PutAsJsonAsync($"/api/v1/accounts/{accountId}/subscriptions/{nonExistentId}/reactivate", (object?)null);
    ...
    [Fact]
    public async Task ReActivateSubscription_ReturnsBadRequestWhenSubscriptionIsActive()
    {
        // Arrange
        ...
        // Act — trying to reactivate an active subscription
        var response = await Client.PutAsJsonAsync($"/api/v1/accounts/{account.Id}/subscriptions/{subscription.Id}/reactivate", (object?)null);
        ...
        var problemDetails = await response.Content.ReadFromJsonAsync<ProblemDetails>();
        problemDetails.Should().NotBeNull();
        problemDetails!.Detail.Should().Be($"Can't re-activate the subscription it is not in {Domain.Entities.SubscriptionStatus.Suspended} state!");
    }

    [Fact]
    public async Task ReActivateSubscription_ReturnsOkAndReactivatesSubscription()
    {
        // Arrange
        var tenantId = Guid.NewGuid();
        var productId = Guid.NewGuid();
