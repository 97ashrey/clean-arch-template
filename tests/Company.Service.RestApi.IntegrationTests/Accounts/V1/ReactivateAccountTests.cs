...
public class ReactivateAccountTests(IntegrationTestWebAppFactory factory) : IntegrationTestBase(factory)
{
    [Fact]
    public async Task ReactivateAccount_ReturnsNotFoundWhenAccountDoesNotExist()
    {
        // Arrange
        var nonExistentId = Guid.NewGuid();

        // Act
        var response = await Client.PutAsJsonAsync($"/api/v1/accounts/{nonExistentId}/reactivate", (object?)null);
    ...
    [Fact]
    public async Task ReactivateAccount_ReturnsBadRequestWhenAccountIsActive()
    {
        // Arrange
        ...
        // Act — trying to reactivate an active account
        var response = await Client.PutAsJsonAsync($"/api/v1/accounts/{account.Id}/reactivate", (object?)null);
        ...
        var problemDetails = await response.Content.ReadFromJsonAsync<ProblemDetails>();
        problemDetails.Should().NotBeNull();
        problemDetails!.Detail.Should().Be($"Can't re-activate the account it is not in {AccountStatus.Suspended} state!");
    }

    [Fact]
    public async Task ReactivateAccount_ReturnsOkAndReactivatesAccount()
    {
        // Arrange
        var tenantId = Guid.NewGuid();
        var invoiceAddress = InvoiceAddress.CreateNew(
        ...
        result.Status.Should().Be(V1Contracts.AccountStatus.Active);
