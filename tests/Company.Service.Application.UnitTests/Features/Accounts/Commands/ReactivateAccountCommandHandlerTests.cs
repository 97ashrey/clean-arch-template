...
public class ReactivateAccountCommandHandlerTests : DbContextTestBase
{
    private readonly ReactivateAccountCommandHandler _sut;
    ...
    {
        // Arrange
        var command = new ReactivateAccountCommand { Id = Guid.NewGuid() };
    ...
    public async Task Handle_WithActiveAccount_ReturnsBadRequestError()
    {
        ...
        var command = new ReactivateAccountCommand { Id = account.Id };
    ...
    [Fact]
    public async Task Handle_WithSuspendedAccount_ReactivatesAndReturnsAccount()
    {
        // Arrange
        var tenantId = Guid.NewGuid();
        var invoiceAddress = CreateInvoiceAddress(tenantId);
